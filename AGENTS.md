...
Self-registration is managed via `RegistrationRequest` (`GoogleSubject`, `Email`, `Status`, `CreatedAt`, `UpdatedAt`, `DecidedAt`). Unknown Google logins do not create immediate user accounts; instead, `GoogleLoginService` creates or updates a pending request and the browser is redirected to the login barrier with a pending notification (`/?registration=pending`). Duplicate logins update changed email addresses without creating duplicates, and previously rejected requests can be re-submitted. A 30-day retention rule purges pending and decided requests automatically. Approval creates a new `User` without entitlements and records an audit entry. Entitled default providers and entitlements are configured administratively afterwards.
...
- `GET /api/admin/registrations`
  - Lists registration requests (optional `status` query filter). Automatically purges requests older than 30 days.
  - Requires the dedicated CLI bearer token.
