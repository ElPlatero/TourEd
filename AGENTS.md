...
It stores Touringen, Harzer Wandernadel (HWN), Malerweg (MW), Schluchtensteig (SST), Heidschnuckenweg (HNW), Harzer Klosterwanderweg (HKW), Bliessteig (BS), and Kellerwaldsteig (KWS) stamping points, hiking tours, tour-to-point relationships, users, and user visits in a SQLite database. The main user-facing feature is showing stamping points on a map and distinguishing visited from unvisited points for a known user.
...
The public privacy notice is served at `Api/wwwroot/datenschutz/index.html` and linked permanently from the map and login barrier. It is available without authentication and carries a `noindex` directive to reduce search-engine discoverability. The notice documents the current Google login, cookies, account/visit storage, IndexedDB offline snapshot, service-worker caching, hosting logs, OpenStreetMap tiles, locally served OpenLayers library, and user-initiated navigation to external provider websites and the public GitHub source repository. Keep it synchronized whenever these data flows or their retention rules change.

The map and login barrier share a compact, touch-accessible legal navigation: `© TourEd · Datenschutz · Impressum · Lizenzen`. The source label links to the public TourEd repository with an accessible AGPL-3.0-only name; OpenStreetMap attribution remains on the map. Static public pages under `/datenschutz/`, `/impressum/`, and `/lizenzen/` share `css/legal.css`, the same navigation and a “Zur Karte” link. Privacy URLs remain supported. The license page includes the complete OpenLayers 5.3.0 BSD notice and a local AGPL text, and explains the individually agreed commercial option without relicensing third-party software or data. All legal pages, their index aliases, shared CSS and local license text belong to the atomic service-worker release; slashless legal navigation redirects to its directory URL even offline. The imprint uses the existing operator name/address and the confirmed info mailbox. Operator name, postal address and email in legal HTML are encoded as numeric character references, remaining selectable, accessible and usable without JavaScript; this is only light source obfuscation, not scraping protection. All legal pages retain `noindex, nofollow, noarchive`; `robots.txt` requests cooperative crawl exclusion for legal routes and their aliases, including nested paths. Disallow does not guarantee deindexing and can prevent crawlers from reading noindex. When hosted under PathBase, the operator must also serve these rules in the origin-root `/robots.txt`. Never publish placeholder operator details.
...
Seeded trail providers (Malerweg, Schluchtensteig, Heidschnuckenweg, Harzer Klosterwanderweg, Bliessteig, Kellerwaldsteig):

- Fixed stamping points across established trails (Malerweg: 8, Schluchtensteig: 6, Heidschnuckenweg: 13, Harzer Klosterwanderweg: 16, Bliessteig: 10, Kellerwaldsteig: 10 unnumbered points), seeded directly via EF Core migrations. The Bliessteig seed records its official CC BY 4.0 route-data provenance; the Kellerwaldsteig seed records its official CC BY-SA 4.0 station-data provenance. Neither offers a GeoJSON export.
- Available without a background network import, but visible only to authenticated users with a matching entitlement.
- Provenance/import metadata fields remain null for Malerweg, Schluchtensteig, Heidschnuckenweg, and Harzer Klosterwanderweg, hiding external source/licence links and public GeoJSON export for those providers. Bliessteig and Kellerwaldsteig display their official source and licence, but intentionally leave `DataImportedAt` unset so TourEd does not offer the optional GeoJSON export.
...
Browser regression tests for concurrent snapshot initialization are documented in `TourEd.Tests/Browser/README.md`; they use two real Chromium tabs sharing one browser context and IndexedDB.

Browser regressions for delayed synchronization confirmations and lease takeover are documented in `TourEd.Tests/Browser/sync-confirmations.md`. They use real Chromium tabs with shared IndexedDB and controlled HTTP responses.

PWA release-coherence browser regressions are documented in `TourEd.Tests/Browser/app-shell-updates.md`. They exercise real service-worker installation, waiting/activation, offline navigation and OAuth exclusions at both `/` and `/toured/`.
...
OpenLayers is vendored from a pinned official upstream commit with provenance and license in `Api/wwwroot/vendor/openlayers/5.3.0`. No frontend build or external library host is required. `TourEd.Tests/Browser/local-openlayers.cjs` checks fresh app-shell installation with foreign hosts blocked, including root and PathBase hosting, anonymous access, authenticated map rendering and offline reload.
